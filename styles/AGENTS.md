...
- `ruang-kendali.css` — Kelas `.rk-*` untuk shell & panel (`.rk-bar` grid `minmax(0,auto) minmax(0,1fr) auto`, `.rk-strip` marquee **di bawah bar** (sticky, Patch 8; satu-satunya animasi berulang), `.rk-konteks` baris PJ OPD, `.rk-main` lebar `var(--rk-lebar)` dari `html[data-lebar]` (penuh/1800/1440, default penuh), `.rk-drawer` 820px / `[data-lebar=lebar]` 1120px, `.rk-nav*` menu radial/baris (Patch 9: `.rk-nav-trig`, `.rk-nav-scrim`, `.rk-nav-gaya`, `.rk-nav li:nth-child` koordinat `--x/--y` per gaya & ponsel — 13 rute, radial r=290/pitch 46, baris ±313; ponsel radial pitch 40, baris kolom 588px, `max-height:720px` → `scale(.82)`; Patch 15), `/pemdi` (`.rk-aspek-atas`, `.rk-dial`, `.rk-stack`/`.rk-leg`, `.rk-ind`, `.rk-ladder .rung.on/.nx`, `.rk-butir`), `/requirement` (`.rk-papan` 3 kolom, `.rk-kartu`, `.rk-ppb-kat`), `/cari` (`.rk-cari-box`, `.rk-hasil`, `.rk-mark`) — Patch 10; `/opd` (`.rk-ubin .ub.s0–s4` peta ubin dengan `--warna`), `/opd/[slug]` (`.rk-strip-butir .sb`, `.rk-pohon .lv`), `/spbe` (`.rk-skala .sumbu/.amb/.tik/.baris/.jalur`, `.rk-domain .dom/.hd/.jalur/.ke`) — Patch 11; `/probis` (`[data-redup]`, `.rk-misi`, `.rk-beban`, `.rk-urusan`, `.rk-lajur .lj`), `/glosarium` (`.rk-glos-kendali`, `.rk-glos`, `.rk-istilah`), `/404` (`.rk-404 .kode`), `a.rk-chip`/`.rk-chip.utama` — Patch 12; `/modul-indikator` (`.rk-modul .hd/.tangga/.isi/.atas/.dua/.krit/.butir`, `.rk-mx`, `.rk-dok`, `.rk-pratinjau`, `.gulir`) — Patch 13; `/asesor` + panel dashboard (`.rk-banding .brs/.jalur i.a|.m|.tgt`, `.rk-verif .sel .ttk`, `.rk-as`, `.rk-tag.t-asesor`) — Patch 14; jembatan `.rk-legacy` tidak lagi dipakai rute mana pun (jaring pengaman); blok **ponsel Patch 15** (`@media ≤860px`: target sentuh ≥40–44px untuk `.rk-chip`, `.rk-seg`, `.rk-btn.kecil`, `.rk-lipat`, `.rk-strip-butir .sb` 24px, `.rk-ladder .rung`, `.rk-verif .sel`; `.rk-ubin` baris 48px); panel lipat (`.rk-panel[data-lipat]`, `.rk-lipat`, `.rk-ringkas`, `.rk-lipat-semua`), `.rk-tabs`/`.rk-lain`/`.rk-btab` dihapus, `.rk-grid` 12 kolom, `.rk-panel`, `.rk-tag`, `.rk-drawer`, `.rk-palet`, `.rk-ppb`, `.rk-lain`), plus **jembatan `.rk-legacy`** yang memetakan token lama (`--primary`, `--gold`, `--bg`, `--surface*`, `--line`, `--muted`, dsb.) ke token rk per tema sehingga halaman lama tampil konsisten tanpa reskin penuh.
- `globals.css` — CSS halaman lama (pemdi, modul-indikator, requirement, opd, probis, spbe, glosarium, cari, 404). **1.158 baris** (Patch 3, 22 Sep 2026 — CSS shell lama sidebar/topbar/bottom-nav/footer/scroll-top/gov-strip/KPI/aspek dihapus; sebelumnya 1.732; tumbuh 2 baris sejak audit 25 Sep 2026). Semua styling di sini.
- Tidak ada module CSS / Tailwind. Font < 11px, emoji ikon, dan `transition: all` (Patch 16) **ditolak** oleh `scripts/cek-ui.mjs` (bagian `npm test`). Daftar periksa lengkap: `desain/PEDOMAN-ANTARMUKA.md`; ukuran nyata di peramban: `npm run audit:ui`.
...
- **Kebijakan motion (sejak 20 Sep 2026 — produksi)**: **tidak ada** animasi dekoratif (blob/blur besar, `backdrop-filter`, pita motif, parallax, 3D transform, count-up, scroll-reveal, ornamen SVG absolut di hero — semua dihapus). **Pengecualian** (keputusan pemilik): (1) running text marquee `.rk-strip` (informasi resmi, bukan dekorasi); (2) **blink pengumuman dashboard** `.rk-pengumuman-teks`/`.rk-peng-popup-teks` (25 Sep 2026 — `rk-peng-kedip` 1,15 s, dimatikan oleh `prefers-reduced-motion`, teks tetap center+bold). Yang diizinkan: `transition` warna/bayangan/border ≤0,3 s, micro-interaction ≤3 px, animasi masuk satu kali ≤0,3 s untuk overlay. `data-reveal`, kelas `reveal`/`aurora`/`d1–d4`, `MotifEmun`, `MarqueeBudaya`, `MotifBackground` sudah dihapus — jangan diperkenalkan kembali.
