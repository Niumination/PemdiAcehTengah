...
- `ruang-kendali.css` — Kelas `.rk-*` untuk shell & panel (`.rk-bar` grid `minmax(0,auto) minmax(0,1fr) auto`, `.rk-strip` marquee **di bawah bar** (sticky, Patch 8; satu-satunya animasi berulang), `.rk-konteks` baris PJ OPD, `.rk-main` lebar `var(--rk-lebar)` dari `html[data-lebar]` (penuh/1800/1440, default penuh), `.rk-drawer` 820px / `[data-lebar=lebar]` 1120px, `.rk-nav*` menu radial/baris (Patch 9: `.rk-nav-trig`, `.rk-nav-scrim`, `.rk-nav-gaya`, `.rk-nav li:nth-child` koordinat `--x/--y` per gaya & ponsel), panel lipat (`.rk-panel[data-lipat]`, `.rk-lipat`, `.rk-ringkas`, `.rk-lipat-semua`), `.rk-tabs`/`.rk-lain`/`.rk-btab` dihapus, `.rk-grid` 12 kolom, `.rk-panel`, `.rk-tag`, `.rk-drawer`, `.rk-palet`, `.rk-ppb`, `.rk-lain`), plus **jembatan `.rk-legacy`** yang memetakan token lama (`--primary`, `--gold`, `--bg`, `--surface*`, `--line`, `--muted`, dsb.) ke token rk per tema sehingga halaman lama tampil konsisten tanpa reskin penuh.
