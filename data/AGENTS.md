...
- `pemdi.json` — evaluasi Pemdi 7 aspek × 20 indikator + 232 butir bukti GT. **Capaian dihitung `scripts/hitung-capaian-pemdi.py` dengan rumus resmi PermenPANRB 8/2026** (Indeks Aspek = Σ(wIj×NIj)/wAi; Indeks = Σ(wA×IA); nilai indikator = level kontinu semua-butir-utama **`diterima`** asesor; indikator eksternal I5/I6/I7/I18 via field `eksternal`). Jangan pakai `update-indeks-aktual.py` (deprecated).
  ...
  - Alur sinkron: `scripts/apply-eval-tahap1.py` (terapkan hasil portal) → `scripts/hitung-capaian-pemdi.py` (indeks) → `scripts/build-draf-prioritas.py` (→ `draf-bukti-prioritas.json`).
