...
| **Patch 22 — Koreksi kecil 25 Sep 2026** | (0) `docs/PANDUAN-AKTIVASI-CMS.md` — langkah aktivasi Neon + 3 env CMS di Vercel, verifikasi, rutinitas ekspor (hold dilepas pemilik 25 Sep). (1) `/modul-indikator`: `CatatanTujuan compact` diberi `className="rk-tujuan"` → di dalam `.rk-grid` merentang penuh (`grid-column: 1 / -1`), margin 0, token `--rk-panel/--rk-ink` — sebelumnya menempati satu kolom grid sehingga tidak presisi dengan layar. (2) Menu navigasi: garis busur putus-putus mode Radial (`.busur`) dan garis vertikal mode Baris (`.rk-nav::before`) **dihapus** — hanya tombol + label. |
| **Patch 21 — Tahap F: pra-uji tugas 3 persona × 5 tugas (25 Sep 2026)** | `scripts/uji-tugas.mjs` (Playwright, `--base=`) menyimulasikan T1–T5 dari `docs/UJI-TUGAS-PEMDI.md` dan memverifikasi jawaban terlihat di layar → `audit/keluaran/uji-tugas.json`; putaran 1: 2/8 lolos, putaran 2: **8/8** (`docs/HASIL-UJI-TUGAS-SIMULASI-25-SEP.md`). Perbaikan: Tugas saya membuka grup pertama yang berisi (OPD tanpa prioritas tinggi) + petunjuk "ketuk untuk membuka"; header tombol **Cetak** (`.rk-kbtn-cetak`, ≥861px, `window.print()`) + `@media print` global (sembunyikan bar/strip/menu/drawer/tombol, buka panel terlipat, satu kolom); footer tombol **Bagikan tautan halaman ini** (`.rk-bagikan`, WA). Sesi dengan peserta nyata tetap wajib — daftar hal yang tak bisa disimulasikan ada di dokumen hasil. |
| **Patch 20 — Tahap E: audit konten & "Tugas saya" (25 Sep 2026)** | Metode audit konten alat kerja: `npm run inventaris` (`scripts/inventaris-konten.mjs`) memindai `pages/` → `docs/INVENTARIS-KONTEN.md` (rute → render → sumber data → panel → tautan aksi; blok JTBD per persona diisi manual, dipertahankan saat regenerasi). `npm run cek:data` (`scripts/cek-data.mjs`, + `test/cekData.test.mjs`) menjaga: umur `data/*.json` >14 hari → peringatan; indeks asesor 1,24 = rata-rata tertimbang aspek; status/prioritas butir dikenal; setiap butir bercatatan mandiri punya PJ yang cocok dengan satu OPD lewat pencocok halaman (`lib/pjButir hitungButirOPD`) — butir yatim = gagal. `npm run cek:ui` ditambahkan sebagai alias. Halaman `/opd/[slug]` ditata ulang jadi **"Tugas saya"** PJ OPD: butir dikelompokkan Minggu ini/Berikutnya/Bila sempat (prioritas), tiap baris menyebut aksi berikutnya (revisi asesor vs siapkan dokumen level) + kebutuhan pertama; bilah status menggantikan strip 232 sel `.sb` 24px; aksi Salin tautan / Bagikan WA / Cetak; daftar per grup dibatasi 8 + "Tampilkan n lagi"; butir dihitung saat build (`butirStatis`, overlay CMS menggantikan di klien) sehingga tanpa "Memuat…"/CLS. Footer `RKShell`: tombol **Laporkan masalah halaman ini** (WA, membawa URL + UA). Hasil audit-ui `/opd/dinas-komunikasi-dan-informatika` 390px: tinggi 6.396 → **3.443**, target <44px 82 → 0 (sisa 3 tombol header 40–42px lebar, pra-ada); 1440px CLS 0,56 → 0,05. |
| **Patch 19 — Tahap D: panjang halaman ponsel (25 Sep 2026)** | Diukur `audit-ui` 390px, tinggi halaman: /probis **16.574 → 2.280**, /dashboard **9.736 → 3.617**, /requirement 8.507 → 4.706, /pemdi 6.208 → 3.978, /asesor 5.952 → 3.094, /opd 6.202 → 2.386, /spbe 3.695 → 2.437; CLS 0 semua rute ponsel; overflow/tindih 0. Mekanisme: `PanelLipat` prop baru **`ponsel="tutup"`** — di ≤860px panel mulai terlipat bila pengguna belum pernah memilih (localStorage kosong); pra-hidrasi dilipat lewat CSS `[data-ponsel='tutup']` sehingga **tanpa layout shift**, setelah hidrasi state React disamakan (`matchMedia`) dan atribut dilepas; desktop tidak berubah. Panel yang dilipat di ponsel: dashboard (asesor, beban, linimasa, prasyarat, ppb, opd — Kompas & Antrean tetap terbuka), /probis (beban, L1, L2), /requirement (P1, panduan), /asesor (tabel, interview), /opd (tabel), /modul-indikator (matriks, dokumen), /spbe (domain, rek, kuat), /pemdi (Aspek 2–7). Ponsel: `.rk-lipat-semua` lengket di bawah strip (Buka semua/Lipat semua selalu terjangkau), transisi lipat dimatikan, kepala panel terlipat tanpa garis/padding bawah, tombol lipat + judul selalu satu baris (aksi turun baris). Belum: `/opd/[slug]` ponsel 6.396px & 82 target `.sb` 24px (strip 232 sel) — akan ditata ulang di Tahap E sebagai halaman "tugas saya" PJ OPD. |
| **Patch 18 — Tahap C: presisi desktop (25 Sep 2026)** | Diukur `audit-ui` 1440px, target <32px: /pemdi 176→**1**, /requirement 163→**1**, /probis 215→**2**, /dashboard 43→17, /asesor 46→16; overflow/tindih tetap 0; tinggi /pemdi 3.283→2.773px. (1) **Hierarki KPI** `.rk-sit` desktop: kolom `2fr + 4×1fr`, angka 1,24 `--rk-fs-7` (64px), 4 kartu lain `--rk-fs-4`; `tabular-nums` untuk angka/kolom. (2) `Situasi ringkas` (prop baru) — bar konteks 1 baris di `/pemdi` (1,24 / 2,50 · H−n · diterima/revisi · antrean · disentuh · "Situasi lengkap →"); strip 5 kartu hanya di `/dashboard`. (3) `/pemdi` 2 kolom → **multikolom CSS** (`.rk-aspek-wrap.dua { columns: 2 }`, `break-inside: avoid`) sehingga kolom kanan tidak kosong 800px; chip level `.rk-ladder .rung b` 2 baris (tidak lagi "Kurang (Merin…"). (4) Target desktop ≥32px: `.rk-lipat`, `.rk-chip`, `.rk-seg.kecil`, `.rk-btn.kecil`, `.rk-kbtn` lipat-semua, `.rk-act` di h2/tabel, `.cm-btn`, tombol beban PJ, tautan rujukan/tujuan; `.sb` strip butir 14→18px (tetap <32: kepadatan disengaja, 232 sel). (5) **URL = state** — `lib/useUrlState.js` (baca `router.query` setelah `isReady`, tulis `router.replace` shallow, nilai default dihapus dari URL): `/antrean` `?q&pri&jenis`, `/requirement` `?tab&jenis`, `/opd` `?level`, `/modul-indikator` `?tab&aspek&level`. Tautan filter kini bisa dikirim ke PJ OPD. Catatan: CLS desktop intermiten `/opd/[slug]` 0,23 dan `/dashboard` 0,07 (tidak terreproduksi saat diukur ulang dengan sumber layout-shift) — diselidiki di Tahap D. |
