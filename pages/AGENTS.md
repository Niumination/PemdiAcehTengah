...
## Purpose
Halaman Next.js Pages Router — entry point untuk **pengguna internal Pemdi** (Tim Koordinasi Pemdi + penanggung jawab OPD). SSG dari `data/*.json`; tidak ada data dinamis/DB sejak reposisi 22 Sep 2026.
...
| `/_app` | `_app.js` | App wrapper — AppShell, canonical + og:url dinamis, meta PWA, JSON-LD, Analytics | AppShell |
...
## Work Guidance
- Halaman baru: buat `.js` di `pages/`, tambahkan link di `Sidebar.js` + label breadcrumb `PETA_JUDUL` di `AppShell.js` (+ `BottomNav.js` bila utama), update DOX
