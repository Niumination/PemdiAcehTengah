...
// ── Fokus level (21 Sep 2026) — satu sumber untuk /pemdi, /modul-indikator, beranda ──
/**
 * Menentukan level mana yang PERLU DITAMPILKAN secara default agar perhatian
 * tim tertuju pada kondisi aktual:
 *   - levelDicapai  : level kontinu tertinggi yang SEMUA butir utamanya diterima asesor
 *                     (sama persis dengan nilaiIndikator — 0 bila L1 belum diterima)
 *   - levelBerikut  : level yang sedang dikejar = levelDicapai + 1 (null bila sudah 5)
 *   - tampil        : himpunan level yang dibuka default = {levelDicapai (jika ≥1), levelBerikut}
 * Level lain disembunyikan (tetap bisa dibuka dengan klik). Indikator eksternal
 * (I5/I6/I7/I18) tidak punya level bukti lokal → hanya levelBerikut = 1 yang dibuka
 * sebagai konteks, dengan flag `eksternal`.
 */
export function fokusLevel(ind) {
  const n = nilaiIndikator(ind);
  const eksternal = n.sumber === 'eksternal';
  const levelDicapai = eksternal ? 0 : n.levelKontinu;
  const levelBerikut = levelDicapai >= 5 ? null : levelDicapai + 1;
  const tampil = new Set();
  if (levelDicapai >= 1) tampil.add(levelDicapai);
  if (levelBerikut) tampil.add(levelBerikut);
  return { levelDicapai, levelBerikut, tampil, eksternal };
}

/** Label peran level relatif terhadap posisi indikator (untuk badge kecil di header level). */
export function peranLevel(level, fokus) {
  if (!fokus) return null;
  if (level === fokus.levelDicapai) return { key: 'dicapai', label: 'Level dicapai', ket: 'Seluruh butir utama level ini sudah diterima asesor' };
  if (level === fokus.levelBerikut) return { key: 'berikut', label: 'Target berikutnya', ket: 'Lengkapi & pastikan seluruh butir level ini diterima agar nilai naik' };
  if (level < fokus.levelDicapai) return { key: 'lewat', label: 'Sudah dilewati', ket: 'Level di bawah level dicapai — sudah terpenuhi' };
  return { key: 'nanti', label: 'Tahap selanjutnya', ket: 'Dinilai setelah level di bawahnya diterima' };
}
