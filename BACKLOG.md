...
- [x] **A9 kunci produksi jadi *sensitive*** — 19 Sep 2026: `SUPABASE_SERVICE_ROLE_KEY` (akses penuh database) dan `IP_HASH_SALT` diubah ke tipe *sensitive* di Vercel. Nilai tidak berubah — dibuktikan `vercel env pull` kini hanya mengembalikan placeholder `[SENSITIVE]`. Hanya `SUPABASE_URL` yang tetap non-sensitive (bukan rahasia)
